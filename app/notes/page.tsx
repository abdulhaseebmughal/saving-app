  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-5xl font-serif font-bold text-amber-900 dark:text-amber-100 mb-2">
            My Notes
          </h1>
          <p className="text-amber-700 dark:text-amber-300 text-lg font-light italic">
            Your personal diary and thoughts collection
          </p>
        </div>

        {/* Color Picker for New Note */}
        <div className="mb-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border-2 border-amber-200 dark:border-amber-700">
          <div className="flex items-center gap-3 mb-4">
            <Plus className="w-5 h-5 text-black" />
            <h2 className="text-xl font-serif font-semibold text-amber-900 dark:text-amber-100">
              Create New Note
            </h2>
          </div>
          <div className="flex flex-wrap gap-3">
            {NOTE_COLORS.map((colorOption) => (
              <Button
                ...
                className="group relative h-16 w-24 rounded-xl border-2 border-gray-300 hover:border-amber-500 transition-all hover:scale-110 hover:shadow-lg"
                style={{ backgroundColor: colorOption.color }}
              >
                <span className="text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: colorOption.textColor }}>
                  {colorOption.name}
                </span>
              </Button>
            ))}
        ...
        {pinnedNotes.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-serif font-bold text-amber-900 dark:text-amber-100 mb-4 flex items-center gap-2">
...
function NoteCard({
  ...
  return (
    ...
    >
      <div
        className="rounded-2xl shadow-xl border-2 border-gray-300 overflow-hidden h-[500px] flex flex-col"
        style={{ backgroundColor: note.color }}
      >
        {/* Header */}
        <div className="p-4 border-b-2 border-amber-900/20 bg-gradient-to-b from-black/5 to-transparent">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <Button
                variant="ghost"
                size="icon"
                className={`h-7 w-7 hover:bg-black/10 ${note.isPinned ? 'text-amber-600' : 'text-black'}`}
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-7 w-7 hover:bg-red-100 text-black hover:text-red-600"
          ...
          {showColorPicker && (
            <div className="absolute top-14 right-4 z-10 bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-3 border-2 border-gray-200">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-transparent border-b-2 border-amber-900/30 pb-1 text-lg font-serif font-bold text-black placeholder:text-black/40 focus:outline-none focus:border-amber-900"
            />
          </div>
        </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto relative">
          <div className="space-y-6">
            {[...Array(15)].map((_, i) => (
              <div key={i} className="border-b border-amber-900/20 pb-6" />
            ))}
          </div>
          <textarea
            value={localContent}
            ...
            className="absolute inset-0 w-full h-full bg-transparent p-4 text-black placeholder:text-black/40 resize-none focus:outline-none font-serif leading-[2.5rem] text-base"
